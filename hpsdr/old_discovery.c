...
static void *discover_receive_thread(void *data);
...
static void discover(struct ifaddrs* iface) {
    ...
    pthread_create(&(discover_thread_id), NULL, &discover_receive_thread, NULL);
    if (!discover_thread_id)
    {
        fprintf(stderr,"thread_new failed on discover_receive_thread\n");
        exit( -1 );
    }
    ...
    pthread_join(discover_thread_id, NULL);
                ...
                if(devices<MAX_DEVICES) {
                    ...
                    case DEVICE_HERMES_LITE:
                        ...
                        if (discovered[devices].software_version < 40) {
                            ...
                            printf("discovered HL2: Gateware Major Version=%d Minor Version=%d\n",buffer[9],buffer[15]);
                        }
                        discovered[devices].frequency_min=0.0;
